...
def train(args, train_loader, model, criterion, criterion_exist, optimizer, epoch):
    ...
    for i, (input, target, crop_input, crop_target, target_exist) in enumerate(train_loader):
        if i >= 100:
            break
        ...
        crop_input_var = torch.autograd.Variable(crop_input)
        crop_target_var = torch.autograd.Variable(crop_target)
        ...
        crop_output, _ = model(crop_input_var)
        crop_loss = criterion(torch.nn.functional.log_softmax(crop_output, dim = 1), crop_target_var)

        loss_tot = (loss + crop_loss) / 2 + loss_exist * 0.1
        ...
        if (i + 1) % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}], LR: {lr:.5f}\t' 'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t' 'Loss {loss.val:.4f} ({loss.avg:.4f})\t' 'Loss Exist {loss_exist.val:.4f} ({loss_exist.avg:.4f})\t'.format(epoch, i, len(train_loader), batch_time = batch_time, data_time = data_time, loss = losses, loss_exist = losses_exist, lr = optimizer.param_groups[-1]['lr']))
...
def validate(args, val_loader, model, criterion, iter, evaluator, best_mIoU, logger = None):
    ...
    for i, (input, target, crop_input, crop_target, target_exist) in enumerate(val_loader):
        if i >= 100:
            break
        ...
        with torch.no_grad():
            ...
            crop_pred = crop_output.data.cpu().numpy().transpose(0, 2, 3, 1)
            crop_pred = np.argmax(crop_pred, axis = 3).astype(np.uint8)

        iou_tot = evaluator(pred, target.cpu().numpy()) + evaluator(crop_pred, crop_target.cpu().numpy())
        loss_tot = (loss + crop_loss) / 2
        
        IoU.update(iou_tot)
        losses.update(loss_tot.data.item(), input.size(0))
...
def main():
    ...
    try:
        ret = model.encoder.load_state_dict(torch.load(args.encoder), strict = False)
    except RuntimeError as e:
        print('[Warning] Ignoring {}'.format(e))
        
    model.decoder.load_state_dict(torch.load(args.est_decoder))
    ...
    train_loader = torch.utils.data.DataLoader(LaneEstDataset(data_path, 
                                                              args.train_list, 
                                                              args.img_width, 
                                                              args.img_height, 
    ...
    val_loader = torch.utils.data.DataLoader(LaneEstDataset(data_path, 
                                                            args.val_list, 
                                                            args.img_width, 
                                                            args.img_height, 
                                                            transform = torchvision.transforms.Compose([tf.GroupNormalize(mean = (input_mean, (0, )), 
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            ...
            save_checkpoint(args, {'epoch': epoch + 1, 'arch': args.arch, 'state_dict': encoder.state_dict(), 'best_mIoU': best_mIoU}, is_best, 'encoder')
            save_checkpoint(args, {'epoch': epoch + 1, 'arch': args.arch, 'state_dict': decoder.state_dict(), 'best_mIoU': best_mIoU}, is_best, 'est_decoder')
