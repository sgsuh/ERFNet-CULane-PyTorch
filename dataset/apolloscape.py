...
if __name__ == "__main__":
    ...
    road_fold_list = glob.glob(os.path.join(root_path, 'road*'))
    road_fold_list.sort()

    for road_fold_path in road_fold_list:
        record_img_fold_list = glob.glob(os.path.join(road_fold_path, 'ColorImage/*'))
        record_img_fold_list.sort()

        record_label_fold_list = glob.glob(os.path.join(road_fold_path, 'Label/*'))
        record_label_fold_list.sort()

        train_num = int(len(record_img_fold_list) * 0.9)
        val_num = len(record_img_fold_list) - train_num

        for i, record_img_fold_path in enumerate(record_img_fold_list):
            img_file_list = glob.glob(os.path.join(record_img_fold_path, '*.png'))
            img_file_list.sort()

            record_label_fold_path = record_label_fold_list[i]

            label_file_list = glob.glob(os.path.join(record_label_fold_path, '*.png'))
            label_file_list.sort()

            for j, img_path in enumerate(img_file_list):
                print(img_path)

                label_path = label_file_list[j]
                ...
                dst_img_path = img_path.split(root_path)[-1]
                dst_label_path = label_path.split(root_path)[-1]

                write_line = dst_img_path + ' ' + dst_label_path + ' ' + str(exist[0]) + ' ' + str(exist[1]) + ' ' + str(exist[2]) + '\n'
